...
/*  Lattice-Boltzmann para la ecuacion de onda con fuente
 *     2
 *     |0
 * 3 --*--1  con w0
 *     |
...
public:
  ...
  double rho(int ix, int iy, bool calculeConLosNew, double sigma);
  ...
  void imprimaseUnaLinea(const char *nombreArchivo, int t);
};
...
double LatticeBoltzmann::rho(int ix, int iy, bool calculeConLosNew, double sigma) {
  int i; double suma;
...
double LatticeBoltzmann::GetSigma(int ix, int iy, int t) {
  double A=10.0, lambda=10, omega=2*M_PI*C/lambda;
  if (ix==Lx/2 && iy==Ly/2)
...
void LatticeBoltzmann::imprimaseUnaLinea(const char *nombreArchivo, int t) {
  ofstream MiArchivo( nombreArchivo );
  double rho0, Jx0, Jy0, sigma;
  int ix=Lx/2;
  for (int iy=0;iy<Ly;iy++) {
...
int main() {
  ...
  ondas.imprimaseUnaLinea("corteCentral.dat",t);
